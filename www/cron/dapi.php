...
while(time()-$czas<59)
{
	...
	$o_curl=new curl();
	$link="http://domobot.monettosa.nstrefa.pl/api/?name=".DB_NAME."&secret=".DB_SECRET."&action=get&last={$last}&limit=1";
	...
	elseif(isset($wynik['data'])) foreach($wynik['data'] as $klucz=>$wartosc)
	{
		...
		else
		{
			...
			else
			{
				...
				$o_curl=new curl();
				$link="http://domobot.monettosa.nstrefa.pl/api/?name=".DB_NAME."&secret=".DB_SECRET."&action=statusUpdate&id={$wartosc->id}&status=2";
				$www=$o_curl->wykonaj($link);
